...
#include <AndromedaClient/Features/CRageBot.hpp>
...
auto CAndromedaClient::OnCreateMove( CCSGOInput* pInput , CUserCmd* pUserCmd ) -> void
{
	...
	if ( pLocalPlayer && pUserCmd )
	{
		g_Aimbot.OnCreateMove( pInput, pUserCmd, pLocalPlayer );
		g_RageBot.OnCreateMove( pInput, pUserCmd, pLocalPlayer );
	}
